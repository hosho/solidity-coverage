...
class App {
  ...
  */
      ...
      shell.ls(`${this.coverageDir}/contracts/**/*.sol`).forEach(file => {
        ...
        if (!this.skipFiles.includes(file) && !this.inSkippedFolder(file)) {
          ...
          if (fs.existsSync(cachedFile)) {
            ...
            // escape an regex symbols that may be in file path
            const absPathPrefix = absolutePathPrefix.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\$&");

            // replace absolute file path prefix with token
            instrumentedContractInfo.contract = instrumentedContractInfo.contract.replace(new RegExp(absPathPrefix, 'g'), '__PATH_PREFIX__');
  ...
  async postProcessPure(env) {
    shell.ls(`${env}/contracts/**/*.sol`).forEach(file => {
      if (this.skipFiles.includes(file) || this.inSkippedFolder(file)) {
        const contractPath = this.platformNeutralPath(file);
        const contract = fs.readFileSync(contractPath).toString();
