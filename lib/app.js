...
class App {
  constructor(config) {
    this.coverageDir = this.platformNeutralPath(process.cwd()) + '/coverageEnv';          // Env that instrumented .sols are tested in
    ...
    // Config
    this.config = config || {};
    this.workingDir = config.dir || this.platformNeutralPath(process.cwd());         // Relative path to contracts folder
  ...
  injectEthEventHandler() {

    const fs = require('fs');
    const EthVmOpsFns = require("ethereumjs-vm/lib/opFns");
    ...
    EthVmOpsFns.LOG = function (...args) {
      ...
      try {
        ...
        let gasLeft = runState.gasLeft.clone();
        let logCount = runState.logs.length;

        ethVmOpsFnsLOG.apply(null, args);

        let gasChange = gasLeft.sub(runState.gasLeft);
        runState.gasLeft.iadd(gasChange);

        if (logCount === runState.logs.length) {
          console.warn(`VM.LOG did not add entry ${logCount}`);
          return;
        }

        let log = runState.logs.pop();
        ...
        if (notCoverageEvent || process.env.NO_EVENTS_FILTER) {
          runState.gasLeft.isub(gasChange)
          runState.logs.push(log);
        }
      }
      catch (err) {
        console.error("Exception in solidity-coverage EthVM.LOG hook:");
        console.error(err);
        throw err;
      }
  ...
  */
  async generateCoverageEnvironment() {
    ...
    try {
      ...
      shell.test('-e', `${this.workingDir}/truffle.js`)
          ? truffleConfig = reqCwd(`${this.workingDir}/truffle.js`)
          : truffleConfig = reqCwd(`${this.workingDir}/truffle-config.js`);
  ...
  async instrumentTarget() {
    ...
    try {
      shell.ls(`${this.coverageDir}/contracts/**/*.sol`).forEach(file => {
        ...
        const canonicalPath = contractPath.replace('/coverageEnv', '');
  ...
  async launchTestrpc() {
    ...
    let truffleConfig = reqCwd(`${this.workingDir}/truffle.js`);
  ...
  sanityCheckContext(){
    ...
    if (shell.test('-e', `${this.coverageDir}`)){
